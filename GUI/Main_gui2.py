...
class Pop_up:
    def error(self,name, latitude  ,longitude):
        self.msg=ctkmessagebox = CTkMessagebox( title='Alert !!', message=f'Name: {name} Location: {longitude} {longitude} Sri Chandrasekarendra Saraswati Vidyapuram Sector-V, Nerul, Navi Mumbai, Maharashtra 400706', fade_in_duration=100, option_1="Cancel", option_2="See in maps")
        ctkmessagebox.after(10000, ctkmessagebox.button_event)

        if self.msg.get()=='See in maps':
            ctkmessagebox.destroy()
            google_url= f"https://www.google.com/maps/place/SIES+Graduate+School+of+Technology/@19.0431532,73.0220929,18.35z/data=!4m6!3m5!1s0x3be7c3db5e2c85cd:0xef26c52d7d73816e!8m2!3d19.042813!4d73.023078!16s%2Fm%2F04gp_tx?entry=ttu"
...
#----------------------------------------Camera components--------------------------------------------
        ...
        self.future1 = self.thread_pool.submit(self.update_time)
        # self.futures_to_wait.append(self.update_time)
    ...
    def call_threaded_cam(self):
        ...
        self.future2 = self.thread_pool.submit(self.camera_connect)
        # self.futures_to_wait.append(self.camera_connect)
        # self.future2.add_done_callback(self.handle_camera_connect_result)
        print("fututres 2 called !!")
...
#             self.net_status.after(2000, self.net)
    ...
    def pixshawk_connect(self):
        ...
        try: 
            if self.drone_controller.connected:
                ...
                self.future3 = self.thread_pool.submit(self.pixshawk_get_info)
                # self.futures_to_wait.append(self.pixshawk_get_info)
    ...
    def alert_status(self):
        if self.my_client:
            ...
            if self.data !=None:
                    ...
                    # self.alert1.start()
                    self.future3 = self.thread_pool.submit(self.play_alert)
                    # self.futures_to_wait.append(self.play_alert)
    ...
    def handle_camera_connect_result(self, future):
        result = future.result()
        print("Handling camera_connect result:", result)
        # Handle the result here

    def wait_for_futures(self):
        try:
            print("Waiting for futures to complete...")
            concurrent.futures.wait(self.futures_to_wait)
            print("All futures completed!")

        except Exception as e:
            print(f"Exception in wait_for_futures: {e}")

    def process_completed_futures(self):
        for completed_future in as_completed(self.futures_to_wait):
            # Handle the completed future
    ...
    def play_alert(self):
        pygame.mixer.init()
        pygame.mixer.music.load('sounds/call-to-attention-123107.mp3')
